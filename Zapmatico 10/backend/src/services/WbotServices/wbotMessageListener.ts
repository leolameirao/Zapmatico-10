...
import { handleTypeBot } from "../TypebotService/TypebotListener";
...
import { addTranscriptionJob } from "../OpenAiServices/Transcription";
...
const verifyMediaMessage = async (
  ...
  const useTranscription = await Setting.findOne({
    where: {
  ...
  if (media.mimetype.split("/")[0] === "audio" && !msg.key.fromMe && useTranscription?.value == 'enabled') {

    const mediaPath = join(__dirname, "..", "..", "..", "public", media.filename);

    addTranscriptionJob(mediaPath, msg.key.id, ticket.companyId.toString());
...
const handleMessage = async (
  ...
  try {
    ...
    if (!ticket.queue && !isGroup && !msg.key.fromMe && !ticket.userId && whatsapp.queues.length >= 1 && whatsapp.useTypebot == false) {
      await verifyQueue(wbot, msg, ticket, ticket.contact);

    }

    if (!ticket.queue && !isGroup && !msg.key.fromMe && !ticket.userId && whatsapp.useTypebot) {

      handleTypeBot(msg, wbot, ticket, ticketTraking)
