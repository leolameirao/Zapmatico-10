...
export default function Options(props) {
  ...
  const [useTranscription, setUseTranscription] = useState("disabled");
  const [loadingUseTranscription, setLoadingUseTranscription] = useState(false);

  const [userEveryoneQueue, setUserEveryoneQueue] = useState("disabled");
  const [loadingUserEveryoneQueue, setLoadingUserEveryoneQueue] = useState(false);
  ...
  useEffect(() => {
    if (Array.isArray(settings) && settings.length) {
      ...
      const transcription = settings.find((s) => s.key === "useTranscription");
      if (transcription) {
        setUseTranscription(transcription.value);
      }

      const tokenopenAi = settings.find((s) => s.key === "openAIToken");
      if (tokenopenAi) {
        setOpenAIToken(tokenopenAi.value);
      }

      const userEveryOne = settings.find((s) => s.key === "userEveryoneQueue");
      if (userEveryOne) {
        setUserEveryoneQueue(userEveryOne.value);
      }
  ...
  const handleSaveOpenAIToken = async (e) => {

    await update({ key: "openAIToken", value: openAIToken, });
    toast.success("Operação atualizada com sucesso.");

  };

  async function handleUseTranscription(value) {
    setUseTranscription(value);
    setLoadingUseTranscription(true);

    if (!openAIToken || openAIToken === "" || openAIToken === null) {

      toast.error("É necessário informar o token do openAI para ativar a transcrição.");
      setLoadingUseTranscription(false);
      return;

    };

    await update({
      key: "useTranscription",
      value,
    });
  ...
  const handleUpdateViewOperator = async (value) => {

    setLoadingUserEveryoneQueue(true);
    setUserEveryoneQueue(value);

    await update({
      key: "userEveryoneQueue",
      value
    });

    toast.success("Operação atualizada com sucesso.");
    setLoadingUserEveryoneQueue(false);
  ...
  return (
    <>
      <Grid spacing={3} container>
        ...
        <Grid xs={12} sm={6} md={4} item>
          <FormControl className={classes.selectContainer}>
            <InputLabel id="useTranscription-type-label">
              Transcrever áudios
            </InputLabel>
            <Select
              labelId="useTranscription-type-label"
              value={useTranscription}
              onChange={async (e) => {
                handleUseTranscription(e.target.value);
              }}
            >
              <MenuItem value={"enabled"}>Sim</MenuItem>
              <MenuItem value={"disabled"}>Não</MenuItem>
            ...
            <FormHelperText>
              {loadingUseTranscription && "Atualizando..."}
            </FormHelperText>
          </FormControl>
        </Grid>
        ...
        <Grid xs={12} sm={6} md={4} item>
          <FormControl className={classes.selectContainer}>
            <InputLabel id="userEveryoneQueue-type-label">
              Operador acessa todos os atendimentos do setor?
            </InputLabel>
            <Select
              labelId="userEveryoneQueue-type-label"
              value={userEveryoneQueue}
              onChange={async (e) => {
                handleUpdateViewOperator(e.target.value);
              }}
            >
              <MenuItem value={"disabled"}>Não</MenuItem>
              <MenuItem value={"enabled"}>Sim</MenuItem>
            ...
            <FormHelperText>
              {loadingUserEveryoneQueue && "Atualizando..."}
            </FormHelperText>
          </FormControl>
        </Grid>
