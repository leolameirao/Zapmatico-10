    ...
    if (messagesList.length > 0) {
      const viewMessagesList = messagesList.map((message, index) => {
        if (!message.fromMe) {
          ...
          console.log('message', message.isComment)

          return (
            <React.Fragment key={message.id}>
              {renderDailyTimestamps(message, index)}
              {renderMessageDivider(message, index)}
              <div className={classes.messageRight} title={message.queueId && message.queue?.name}
                style={{ backgroundColor: `${message.isComment ? '#c8f7f5' : '#dcf8c6'}`,
                paddingBottom: message.isComment ? '20px' : '0', }}
              >
                {!message.isComment && (
                  <IconButton
                    variant="contained"
                ...
                >
                  ...
                  {message.quotedMsg && renderQuotedMessage(message)}
                  <MarkdownWrapper isComment={message.isComment} >{(message.mediaType === 'contactMessage' || message.mediaType === 'contactsArrayMessage') ? ' ' : message.body}</MarkdownWrapper>
                  {/* <MarkdownWrapper>{message.body}</MarkdownWrapper> */}
                  <span className={classes.timestamp}>

                    {message.isComment ? "Comentário " + format(parseISO(message.createdAt), "HH:mm") : format(parseISO(message.createdAt), "HH:mm")}
